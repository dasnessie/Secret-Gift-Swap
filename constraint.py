...
def get_probability_from_constraints(
    constraints: list[Constraint], giver: Participant, giftee: Participant
) -> float:
    ...
    for c in constraints:
        ...
        elif c.giver == giftee and c.giftee == giver:
            if c.probability_level == "never":
                result = "never"
    return matching_probabilities[result]


def get_used_constraint_levels_from_constraints(constraints: list[Constraint]):
...
def get_all_probability_values_from_constraints(constraints: list[Constraint]):
